...
def Pergunta_4():
    ...
    consulta_sql = "SELECT dos.NOME_ORGAO_SUPERIOR, AVG(VALOR_CONVENIO) as valor_convenio FROM dim_orgao_superior as dos inner join fatoconvenios as fc inner join dim_ug_concedente as ugc on ugc.sk_ug_concedente = fc.k_UG_Concedente inner join dim_orgao_concedente as doc on doc.sk_orgao_concedente = fc.k_Orgao_Concedente inner join dimdata as dt on dos.sk_orgao_superior = fc.k_Orgao_Superior and dt.keyData = fc.k_Data and dt.ano_id >= "+ filtro_2 + " and dt.ano_id <= " + filtro_3 +" inner join dim_municipio as dm on dm.sk_municipio = fc.k_Municipio and dm.UF ="+"'"+str(filtro_1)+"'"
    ...
    if filtro_4 != None:
        consulta_sql_2 += " and dos.NOME_ORGAO_SUPERIOR ="+"'"+filtro_4+"'"
